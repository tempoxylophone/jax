...
def operand_info(
  ...
  # lhs_shape[2:] = tile_shape(lhs_shape[2:],  (64, tile_k))
  # rhs_shape[2:] = tile_shape(rhs_shape[2:], (tile_k, tile_k))
  rhs_tiling = (lhs_info.elems_128b, rhs_info.elems_128b) # if rhs_transpose else (lhs_info.elems_128b, rhs_info.elems_128b)
...
class WGMMATF32x3Impl:
  ...
  @staticmethod
  def smem_shape_extra(lhs: MMAOperandInfo, rhs: MMAOperandInfo) -> dict[str, jax.ShapeDtypeStruct]:
    return {"lhs_err": lhs.shape[2:], "rhs_err": rhs.shape[2:]}
...
class WGMMACvtRhsImpl:
  ...
  @staticmethod
  def smem_shape_extra(lhs: MMAOperandInfo, rhs: MMAOperandInfo) -> dict[str, jax.ShapeDtypeStruct]:
    # if lhs.shape[-1] != 64 or rhs.shape[-1] != 64 :
    #   raise NotImplementedError(f"Only effectively 16bit supported atm {(lhs.dtype, rhs.dtype)}")
  ...
  def wgmma(
    ...
    # assert ir.MemRefType(a_slice.type).shape[1] == cvt_ty.shape[0], (a_slice.type, cvt_ty)
...
def verify(
  ...
  with mlir.make_ir_context(), ir.Location.unknown():
    ...
    print(f"{tera_flops=},{tera_flops_ref=}")
