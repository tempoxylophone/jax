...
def make_array_from_process_local_data(
  ...
  well, see test_util.get_process_index_and_count for example how to find
  out what shard index current process should be loading.
  ...
  See sharding_impl.num_addressable_indices for more details how to handle
  general case.

  If the shape matches global shape, each device slice will just lookup
  the slice in the host data. In the latter case the global slice of each
  device will be mapped into local  slice of this host array. For example,
  if given process only addresses slices (8, 12) and  (24, 28), then
  these slices will be mapped into (0, 4) and (4, 8) of the local array.
  ...
  but you would need to compute the size and the content ofprocess-local data
  correctly to satisfy the replication constraints. (See
  test_util.get_process_index_and_count for more details).
  ...
  >>> per_host_batch_slice = global_shape[0] // 4
  ...
  ...                         batch_size=per_host_batch_slice)
  ...
  for i, (data_dim, global_dim) in enumerate(
    ...
    if data_dim != global_dim:
      process_slice = num_addressable_indices(sharding, global_shape, i)
